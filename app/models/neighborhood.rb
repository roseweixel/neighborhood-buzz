...
class Neighborhood < ActiveRecord::Base
  ...
  #     flickr_places = flickr.places.find(:query => "#{name}, New York")
  #     place_ids[neighborhood.name] = flickr_places[0].place_id
  #   end
  #   place_ids
  # end
  ...
  def self.get_photo_urls_for_photoless_neighborhoods
    photo_urls = {}
    FlickRaw.api_key = FLICKR_API_KEY
    FlickRaw.shared_secret = FLICKR_SHARED_SECRET
    NEIGHBORHOODS_WITHOUT_PHOTOS.each do |name|
      flickr_places = flickr.places.find(:query => "#{name}, New York")
      #binding.pry
      flickr_places.each do |place|
        ...
        photo_hash = flickr.photos.search(:text => name, :place_id => place_id, :per_page => '1', :extras => 'url_o')
        #binding.pry
        if photo_hash.size >= 1 && photo_hash[0]["url_o"]
          photo_url = photo_hash[0]["url_o"]
          if photo_url
            photo_urls[name] = photo_url
            break
          end
        end
      end
